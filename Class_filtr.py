        ...
        for i in self.dict_table:
            if self.dict_table[i].isVisible():
                ...
                for j in range(len(list_items)):
                    self.list_check.setItem(j, 0, QtWidgets.QTableWidgetItem( list_items[j]))
                    item = QtWidgets.QTableWidgetItem()
                    item.setFlags(QtCore.Qt.ItemIsUserCheckable | QtCore.Qt.ItemIsEnabled)
                    item.setCheckState(QtCore.Qt.Unchecked) #Unchecked #Checked
                ...
                menu.exec_(QtCore.QPoint(self.shift(cursor_position.x()), cursor_position.y()))
    ...
    def change_string_find(self):
        ...
        if self.input_operator.text() == '==':
            ...
            for i in range(self.list_check.rowCount()):
                ...
                if self.list_check.item(i, 0).text()[:len_word] != self.input_find.text():
